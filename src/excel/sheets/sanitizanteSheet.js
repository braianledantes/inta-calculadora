...
export function addSanitizanteSheet(workbook, planesSanitizantes, valorDolar) {
  ...
  planesSanitizantes.forEach((plan, indexPlan) => {
    ...
    plan.tratamientos.forEach((tratamiento, indexTratamiento) => {
      ...
      tratamiento.productos.forEach((producto, indexProducto) => {
        ...
        // Fórmulas
        row.getCell(10).value = { formula: `F${r} * H${r}` };         // Cant. por ha
        row.getCell(11).value = { formula: `J${r} * C1 * E${r}` };    // Costo por ha

        // Verificar si es el último producto del último tratamiento
        ...
        // Estilo fila tipo tractor para columnas C a L
        const isEven = (row.number - 3) % 2 === 0;
        const style = isEven ? tractorRowStyle : generalCellStyle;

        for (let col = 3; col <= 12; col++) {
          if (col === 12 && esUltimoTratamiento && esUltimoProducto) continue; // Evitar sobrescribir estilo especial
          const cell = row.getCell(col);
          applyStyle(cell, style);
        }

        // Estilo especial para columnas A y B
        [1, 2].forEach(col => {
          const cell = row.getCell(col);
          applyStyle(cell, {
            ...tractorRowStyle,
        ...
        // Estilo y fórmula para TOTAL solo en la última celda del plan
      ...
      if (tratamiento.productos.length > 1) {
        sheet.mergeCells(`B${tratamientoStartRow}:B${currentRow - 1}`);
      }
    });
  ...
  // Estilo general a celdas sin fill y sin border
